...
import {
    ...
    P as Me,
    k as Qe
} from "../chunks/singletons.c7bd1800.js";
import {
    u as Ze
...
function rt(n, r) {
    const l = new URL(n);
    for (const c of at) Object.defineProperty(l, c, {
        get() {
            return r(), n[c]
        },
        enumerable: !0,
...
window.fetch = (n, r) => ((n instanceof Request ? n.method : (r == null ? void 0 : r.method) || "GET") !== "GET" && ae.delete(Ee(n)), Be(n, r));
const ae = new Map;
...
function ft(n, r) {
    ...
    if (c != null && c.textContent) {
        let {
            body: h,
            ...w
        } = JSON.parse(c.textContent);
        const S = c.getAttribute("data-ttl");
        return S && ae.set(l, {
            body: h,
            init: w,
            ttl: 1e3 * Number(S)
        }), c.getAttribute("data-b64") !== null && (h = lt(h)), Promise.resolve(new Response(h, w))
    }
...
function ut(n, r, l) {
    if (ae.size > 0) {
        const c = Ee(n, l),
            h = ae.get(c);
        if (h) {
            if (performance.now() < h.ttl && ["default", "force-cache", "only-if-cached", void 0].includes(l == null ? void 0 : l.cache)) return new Response(h.body, h.init);
            ae.delete(c)
        }
    }
...
function Ee(n, r) {
    ...
    if (r != null && r.headers || r != null && r.body) {
        const h = [];
        r.headers && h.push([...new Headers(r.headers)].join(",")), r.body && (typeof r.body == "string" || ArrayBuffer.isView(r.body)) && h.push(r.body), c += `[data-hash="${ct(...h)}"]`
    }
    return c
}
const dt = /^(\[)?(\.\.\.)?(\w+)(?:=(\w+))?(\])?$/;
...
function pt(n) {
    const r = [];
    return {
        pattern: n === "/" ? /^\/$/ : new RegExp(`^${gt(n).map(c=>{const h=/^\[\.\.\.(\w+)(?:=(\w+))?\]$/.exec(c);if(h)return r.push({name:h[1],matcher:h[2],optional:!1,rest:!0,chained:!0}),"(?:/(.*))?";const w=/^\[\[(\w+)(?:=(\w+))?\]\]$/.exec(c);if(w)return r.push({name:w[1],matcher:w[2],optional:!0,rest:!1,chained:!0}),"(?:/([^/]+))?";if(!c)return;const S=c.split(/\[(.+?)\](?!\])/);return"/"+S.map((y,b)=>{if(b%2){if(y.startsWith("x+"))return ve(String.fromCharCode(parseInt(y.slice(2),16)));if(y.startsWith("u+"))return ve(String.fromCharCode(...y.slice(2).split("-").map(O=>parseInt(O,16))));const u=dt.exec(y);if(!u)throw new Error(`
            Invalid param: $ {
                y
            }.Params and matcher names can only have underscores and alphanumeric characters.
            `);const[,j,D,C,N]=u;return r.push({name:C,matcher:N,optional:!!j,rest:!!D,chained:D?b===1&&S[0]==="":!1}),D?"(.*?)":j?"([^/]*)?":"([^/]+?)"}return ve(y)}).join("")}).join("")}/?$`),
        params: r
    }
}
...
function mt(n, r, l) {
    ...
    for (let m = 0; m < r.length; m += 1) {
        const y = r[m];
        let b = h[m - S];
        if (y.chained && y.rest && S && (b = h.slice(m - S, m + 1).filter(u => u).join("/"), S = 0), b === void 0) {
            y.rest && (c[y.name] = "");
            continue
        }
        if (!y.matcher || l[y.matcher](b)) {
            ...
            const u = r[m + 1],
                j = h[m + 1];
            u && !u.rest && u.optional && j && y.chained && (S = 0), !u && !j && Object.keys(c).length === w.length && (S = 0);
            continue
        }
...
async function vt(n, r) {
    var l;
    for (const c in n)
        if (typeof((l = n[c]) == null ? void 0 : l.then) == "function") return Object.fromEntries(await Promise.all(Object.entries(n).map(async ([h, w]) => [h, await w])));
    return n
}
class ne {
    constructor(r, l) {
        this.status = r, typeof l == "string" ? this.body = {
            message: l
...
function xt(n, r) {
    ...
    let u = {
        ...
        T = !1,
        G = !1,
        F, U = (je = history.state) == null ? void 0 : je[M];
    ...
    function Pe(e) {
        ...
        i && i.remove(), H = e.props.page, F = new n.root({
            target: r,
            props: { ...e.props,
                stores: J,
                components: m
    ...
    async function X({
        ...
        for (let p = 0; p < Math.max(t.length, u.branch.length); p += 1) {
            const d = t[p],
                P = u.branch[p];
            (d == null ? void 0 : d.data) !== (P == null ? void 0 : P.data) && (L = !0), d && (_ = { ..._,
                ...d.data
            }, L && (v.props[`data_${k}`] = _), k += 1)
        }
    ...
    async function ue({
        ...
        if ((_ = v.universal) != null && _.load) {
            ...
            const p = {
                route: new Proxy(a, {
                    get: (d, P) => (g.route = !0, d[P])
                }),
                params: new Proxy(o, {
                    get: (d, P) => (g.params.add(P), d[P])
                }),
                ...
                async fetch(d, P) {
                    ...
                    const q = new URL($, t);
                    return A(q.href), q.origin === t.origin && ($ = q.href.slice(t.origin.length)), D ? ut($, q.href, P) : ft($, P)
                },
                setHeaders: () => {},
                depends: A,
                parent() {
        ...
        return {
            ...
            slash: t.pathname === V || t.pathname === V + "/" ? "always" : ((k = v.universal) == null ? void 0 : k.trailingSlash) ? ? (s == null ? void 0 : s.slash)
        }
    }
    ...
    async function pe({
        ...
        s.forEach(E => E == null ? void 0 : E().catch(() => {})), v.forEach(E => E == null ? void 0 : E[1]().catch(() => {}));
        let _ = null;
        const L = u.url ? e !== u.url.pathname + u.url.search : !1,
            k = u.route ? a.id !== u.route.id : !1;
        let A = !1;
        const p = v.map((E, R) => {
            var K;
            const x = u.branch[R],
                I = !!(E != null && E[0]) && ((x == null ? void 0 : x.loader) !== E[1] || Ue(A, k, L, (K = x.server) == null ? void 0 : K.uses, o));
            return I && (A = !0), I
        });
        if (p.some(Boolean)) {
            try {
        ...
        let P = !1;
        const $ = v.map(async (E, R) => {
            var he;
            if (!E) return;
            const x = u.branch[R],
                I = d == null ? void 0 : d[R];
            if ((!I || I.type === "skip") && E[1] === (x == null ? void 0 : x.loader) && !Ue(P, k, L, (he = x.universal) == null ? void 0 : he.uses, o)) return x;
            ...
            return ue({
                ...
                parent: async () => {
                    var Te;
                    const Ce = {};
                    for (let ge = 0; ge < R; ge += 1) Object.assign(Ce, (Te = await $[ge]) == null ? void 0 : Te.data);
                    return Ce
                },
        ...
        for (let E = 0; E < v.length; E += 1)
            if (v[E]) try {
                ...
                let x = 500,
                    I;
                if (d != null && d.includes(R)) x = R.status ? ? x, I = R.error;
                else if (R instanceof ne) x = R.status, I = R.body;
                else {
    ...
    async function ce({
        ...
        const A = U;
        v(), O = !0, D && J.navigating.set(k.navigation), W = g;
        let p = L && await pe(L);
        if (!p) {
        ...
        if (e = (L == null ? void 0 : L.url) || e, W !== g) return k.reject(new Error("navigation was aborted")), !1;
        if (p.type === "redirect")
            if (o >= 20) p = await ie({
                status: 500,
        ...
        else(($ = p.props.page) == null ? void 0 : $.status) >= 400 && await J.updated.check() && await B(e);
        if (S.length = 0, G = !1, N = !0, be(A), ke(A), (q = p.props.page) != null && q.url && p.props.page.url.pathname !== e.pathname && (e.pathname = (E = p.props.page) == null ? void 0 : E.url.pathname), a) {
            const R = a.replaceState ? 0 : 1;
            if (a.state[M] = U += R, history[a.replaceState ? "replaceState" : "pushState"](a.state, "", e), !a.replaceState) {
                let x = U + 1;
        ...
        if (y = null, D) {
            ...
            const R = (await Promise.all(b.on_navigate.map(x => x(k.navigation)))).filter(x => typeof x == "function");
            if (R.length > 0) {
                let x = function() {
                    b.after_navigate = b.after_navigate.filter(I => !R.includes(I))
                };
                R.push(x), b.after_navigate.push(...R)
            }
            F.$set(p.props)
        } else Pe(p);
        const {
        ...
        !t && !P && Se(), C = !0, p.props.page && (H = p.props.page), O = !1, s === "popstate" && Re(U), k.fulfil(void 0), b.after_navigate.forEach(R => R(k.navigation)), J.navigating.set(null), N = !1
    }
    ...
    function Xe() {
        ...
        w.addEventListener("mousedown", i), w.addEventListener("touchstart", i, {
            passive: !0
        });
        ...
        function a() {
            ...
            for (const s of w.querySelectorAll("a")) {
                ...
                if (g || v) continue;
                const _ = le(s);
                _.reload || (_.preload_code === Me.viewport && t.observe(s), _.preload_code === Me.eager && oe(se(f)))
            }
        }
    ...
    return {
        ...
        _start_router: () => {
            ...
            history.scrollRestoration = "manual", addEventListener("beforeunload", t => {
            ...
            }), (i = navigator.connection) != null && i.saveData || Xe(), w.addEventListener("click", t => {
                var A;
                ...
                const v = le(o);
                if (!(o instanceof SVGAElement) && a.protocol !== location.protocol && !(a.protocol === "https:" || a.protocol === "http:") || g) return;
                if (s || v.reload) {
                    $e({
                        url: a,
                        type: "link"
                ...
                const [L, k] = a.href.split("#");
                if (k !== void 0 && L === location.href.split("#")[0]) {
                    if (u.url.hash === a.hash) {
                        t.preventDefault(), (A = o.ownerDocument.getElementById(k)) == null || A.scrollIntoView();
                        return
                    }
                ...
                p && A.append(p, (a == null ? void 0 : a.getAttribute("value")) ? ? ""), f.search = new URLSearchParams(A).toString(), ce({
                    url: f,
                    scroll: _ ? te() : null,
                    keepfocus: v ? ? !1,
                    redirect_count: 0,
                    details: {
                        state: {},
                        replaceState: k ? ? f.href === location.href
                    },
                    nav_token: {},
                    accepted: () => {},
                    blocked: () => {},
                ...
                T && (T = !1, history.replaceState({ ...history.state,
                    [M]: ++U
                }, "", location.href))
            });
        ...
        _hydrate: async ({
            ...
            try {
                const _ = t.map(async (A, p) => {
                        const d = s[p];
                        return d != null && d.uses && (d.uses = Ye(d.uses)), ue({
                            loader: n.nodes[A],
                            url: g,
                            params: o,
                            route: a,
...
async function He(n, r) {
    var h;
    const l = new URL(n);
    l.pathname = st(n.pathname), n.pathname.endsWith("/") && l.searchParams.append(Et, "1"), l.searchParams.append(St, r.map(w => w ? "1" : "0").join(""));
    const c = await Be(l.href);
    if ((h = c.headers.get("content-type")) != null && h.includes("text/html") && await B(n), !c.ok) throw new ne(c.status, await c.json());
...
`);
                ...
                if (O.type === "data")(j = O.nodes) == null || j.forEach(T => {
                    (T == null ? void 0 : T.type) === "data" && (T.uses = Ye(T.uses), T.data = b(T.data))
                }), w(O);
            ...
            setTimeout(() => {
                if (c.rangeCount === h.length) {
                    for (let w = 0; w < c.rangeCount; w += 1) {
                        const S = h[w],
                            m = c.getRangeAt(w);
                        if (S.commonAncestorContainer !== m.commonAncestorContainer || S.startContainer !== m.startContainer || S.endContainer !== m.endContainer || S.startOffset !== m.startOffset || S.endOffset !== m.endOffset) return
                    }
                    c.removeAllRanges()
                }
            })
